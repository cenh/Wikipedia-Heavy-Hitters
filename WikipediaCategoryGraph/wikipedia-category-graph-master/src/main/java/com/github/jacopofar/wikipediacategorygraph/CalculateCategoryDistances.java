...
/**
 * Calculate the distances of each category from the chosen ones, with these
 * criteria: 1. a step from a category to a sub-category costs 1 2. a step from
 * a category to a container category costs 3 3. the distance between two
 * categories is the path with the lower cost possible between them
 */
	...
	public static void main(String args[]) {
		...
		try (Transaction tx = graphDb.beginTx()) {
			for (String categoryName : args[1].split("\\|")) {
				...
				ResourceIterator<Node> matcher = graphDb.findNodes(categoryLbl, "name", categoryName);
		...
		while (frontier.size() > 0) {
			...
			for (DistanceLabel dstart : frontier) {
				try (Transaction tx = graphDb.beginTx()) {
					...
					for (Node contained : dstart.getContained()) {
						...
						// if the node has changed (so it's a new node or has been reached with a lower
						// distance) add it to the next frontier
						if (candidate.mark()) {
			...
			System.out.println(
					"Iteration number " + iterations + " finished in " + (System.currentTimeMillis() - startTime) / 1000
							+ "s, " + nextFrontier.size() + " nodes pending for the next iteration");
